...
頁寬 1260px，上限內保留 96px 外側總留白；1100px 以下首頁改 64px、760px 以下 40px。首頁主要雙欄在 760px 以下堆疊，唱片四欄改兩欄。導覽在手機換成第二列，直接顯示連結，不藏進選單。圖片明確寬高並保留比例；照片用 CSS object-fit，保留來源原圖，但實際下載的是 `<picture>` 的 AVIF／WebP 響應式版本，主視覺以 preload 提前取得。首頁與工具頁之間採 cross-document View Transitions，導覽與品牌字標在兩頁間平滑過渡，不支援的瀏覽器一般跳轉。Spotify 播放器按需載入，固定 352px 高並保留外連作為後備。
...
- 首頁動態：主視覺依序進場（標語逐行從遮罩滑出、獅子工作桌縮放浮現後緩慢漂浮），獅子場景以 `docs/lion-art.md` 的圖層活起來：頭部微點、眼睛跟隨游標並偶爾張望、尾巴搖擺、植物輕晃、燈泡與檯燈呼吸發光、耳機飄出音符；細指標時各圖層依景深朝游標微幅視差；捲動時主視覺以 CSS scroll-driven animation 淡出，不支援的瀏覽器由 `--lp-scroll` 後備。各區塊以 `data-reveal` 揭示（rise／blur／scale／tilt），時間線連線生長、專長圖示描邊、價值底線與獎項貼紙分層出現。桌面導覽固定於頂端並在捲動後收窄，黃色標記指出所在區塊；創作領域改為緩慢跑馬燈，hover 暫停。瀏覽器卡與證照卡採 3D 傾斜。減少動態偏好下全部關閉並顯示靜態版面。
- 首頁互動細節：主視覺柔光依台北時間分早晨、白天、傍晚、夜晚換色，夜晚檯燈與螢幕更亮；好理家在的瀏覽器模擬視窗在支援 scroll-driven animation 的瀏覽器隨捲動自行組裝（網址打字、標誌描邊、chip 彈入、獎項蓋章），其他瀏覽器沿用時間揭示；唱片 hover 時黑膠從封面滑出並旋轉；主要按鈕在細指標裝置微幅吸向游標（上限 9px）；唱片、報導列與工具街預覽 hover 時出現跟隨游標的黃色標籤（▶ PLAY／READ ↗／逛逛 ↗），原生游標保留；聯繫按鈕 hover 箭頭化為紙飛機起飛；創作領域跑馬燈每項配一個線條圖示。
