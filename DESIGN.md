...
- 首頁動態：主視覺依序進場（標語逐行從遮罩滑出、獅子工作桌縮放浮現後緩慢漂浮），細指標時場景與便條朝游標微幅視差；捲動時主視覺以 CSS scroll-driven animation 淡出，不支援的瀏覽器由 `--lp-scroll` 後備。各區塊以 `data-reveal` 揭示（rise／blur／scale／tilt），時間線連線生長、專長圖示描邊、價值底線與獎項貼紙分層出現。桌面導覽固定於頂端並在捲動後收窄，黃色標記指出所在區塊；創作領域改為緩慢跑馬燈，hover 暫停。瀏覽器卡與證照卡採 3D 傾斜。減少動態偏好下全部關閉並顯示靜態版面。
